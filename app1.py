...
# 配方建议页面
    ...
    target_loi = st.number_input("目标 LOI 值", min_value=10.0, max_value=50.0, value=25.0, step=0.1)
    ...
    if st.button("生成推荐配方"):
        with st.spinner("🔍 正在优化配方..."):
            ...
            hof = tools.HallOfFame(10)  # 获取前 10 个最优个体
            ...
            for best in hof:
                ...
                # 单位转换逻辑
                if unit_type == "质量 (g)" and total > 0:
                    recipe = {k: (v / 100) * total for k, v in recipe.items()}
                elif unit_type == "质量分数 (wt%)":
                    pass  # 已经是百分比形式
                elif unit_type == "体积分数 (vol%)":
                    recipe = {k: v for k, v in recipe.items()}  # 直接使用质量分数作为体积分数
            ...
            recipe_df.index += 1  # 从 1 开始编号
            recipe_df.index.name = '配方编号'
            ...
            for _, row in recipe_df.iterrows():
                ...
                predictions.append(predicted_loi)
            
            recipe_df['预测 LOI 值'] = [f"{p:.2f}%" for p in predictions]
