...
if page == "性能预测":
    ...
    if target_loi < 10 or target_loi > 40:
        ...
        if generate_button:
            ...
            pp_index = feature_names.index("PP") if "PP" in feature_names else None  # 检查是否有PP列
            if pp_index is None:
                pp_index = feature_names.index("PP")  # 如果没有，手动添加
            ...
            def evaluate(ind):
                ind = np.clip(ind, 0, None)
                if pp_index is not None and ind[pp_index] <= max([x for i, x in enumerate(ind) if i != pp_index]):
                    return 1e6,
            ...
            for ind in hof:
                ...
                if pp_index is not None and norm[pp_index] <= max([x for i, x in enumerate(norm) if i != pp_index]):
                    continue
