...
# 配方建议页面
    ...
    if sub_page == "配方优化":
        ...
        with st.expander("点击查看参考样本"):
            ...
            1. **样本1**：无添加剂
            - 输入数据：[产品质量指标_Sn%: 19.2, 添加比例: 0, 一甲%: 32, 黄度值_3min: 5.36, 黄度值_15min: 6.29, 黄度值_18min: 7.57, 黄度值_21min: 8.57, 黄度值_24min: 10.26]  
            - 预测结果：无推荐添加剂
    
            2. **样本2**：氯化石蜡
            - 输入数据：[产品质量指标_Sn%: 18.5, 添加比例: 3.64, 一甲%: 31.05, 黄度值_3min: 5.29, 黄度值_15min: 6.83, 黄度值_18min: 8.00, 黄度值_21min: 9.32, 黄度值_24min: 11.40]
            - 预测结果：氯化石蜡
    
            3. **样本3**：EA15（市售液体钙锌稳定剂）
            - 输入数据：[产品质量指标_Sn%: 19, 添加比例: 1.04, 一甲%: 31.88, 黄度值_3min: 5.24, 黄度值_15min: 6.17, 黄度值_18min: 7.11, 黄度值_21min: 8.95, 黄度值_24min: 10.33]
            - 预测结果：EA15（市售液体钙锌稳定剂）
            """)
